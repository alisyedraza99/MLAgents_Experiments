...
public class TablePositionRandomizer : MonoBehaviour
{
    ...
    public BoxCollider destCollider;
    ...
    public void Move()
    {
        ...
        Vector3 tableCenter = destCollider.bounds.center;//tableBounds.center; //make problem easier temporarily
        Vector3 newPos;
        if (destCollider != null)
        {
            do
            {
                ...
                newPos = new Vector3(x, targetY, z);
            }
            while (destCollider.bounds.Contains(newPos));
        }
        else
        {
            float x = tableCenter.x + tableTopPoint.x;
            float z = tableCenter.z + tableTopPoint.y;
            newPos = new Vector3(x, targetY, z);
            transform.position = newPos;// new Vector3(x, targetY, z);
        }
